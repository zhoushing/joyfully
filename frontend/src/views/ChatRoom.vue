    ...
    <el-row>
      <el-col :span="4">
        <el-card style="width: 300px; height: 300px; color: #333">
          ...
          <div style="padding: 10px 0" v-for="user in users" :key="user.nickname">
            <span>{{ user.nickname }}</span>
            <i class="el-icon-chat-dot-round" style="margin-left: 10px; font-size: 16px; cursor: pointer"
               @click="chatUser = user.nickname"></i>
            <span style="font-size: 12px;color: limegreen; margin-left: 5px" v-if="user.nickname === chatUser">chatting...</span>
          </div>
        </el-card>
      </el-col>
  ...
  methods: {
    init() {
      ...
      if (typeof (WebSocket) == "undefined") {
        ...
        let socketUrl = "ws://" + window.server.url + ":"+ window.server.port +"/imserver/" + nickname;
        if (socket != null) {
          socket.close();
          socket = null;
        }
        ...
        socket.onmessage = function (msg) {
          ...
          if (data.users) {
            ...
            _this.users = data.users.filter(user => user.nickname !== nickname)
          } else {
    ...
    send() {
      ...
      if (!this.text) {
        ...
        if (typeof (WebSocket) == "undefined") {
          ...
          let message = {
            from: this.user.nickname, to: this.chatUser, text: this.text
          }
